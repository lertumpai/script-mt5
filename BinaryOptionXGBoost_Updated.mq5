//+------------------------------------------------------------------+
//|                              BinaryOptionXGBoost_Updated.mq5 |
...
//--- Include Updated XGBoost Signal Library
...
int OnInit()
{
    Print("=== Binary Option XGBoost Updated EA Started ===");
    ...
    // Initialize Updated XGBoost Library
    InitXGBoostLibrary();
...
{
    Print("=== Binary Option XGBoost Updated EA Stopped ===");
    Print("Reason: ", reason);
}
...
void OnTick()
{
    ...
    // ใช้ Updated Library function หลัก
    ...
    if(signal != 0)
    {
        if(signal == 1)
        {
            Print("=== SIGNAL: CALL (Updated from OHLC Data) ===");
            if(InpEnableTrading)
                PlaceBinaryOption(ORDER_TYPE_BUY, "CALL");
        }
        else if(signal == -1)
        {
            Print("=== SIGNAL: PUT (Updated from OHLC Data) ===");
            if(InpEnableTrading)
...
void PlaceBinaryOption(ENUM_ORDER_TYPE order_type, string signal_type)
{
    ...
    request.comment = "XGBoost Updated " + signal_type;
